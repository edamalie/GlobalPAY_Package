...
class WC_Gateway_GlobalPAY extends WC_Payment_Gateway {
    ...
    public function handle_itn_request($data) {
        ...
        switch ($this->confirmTransaction($returnedRef)) {
            case 'success':

                $this->log( '- Complete' );
                $order->add_order_note( __( 'GlobalPAY payment completed', 'woocommerce-gateway-globalpay' ) );
                ...
                //if ( $this->send_debug_email ) {
                    ...
                    $body =
                        ...
                        "GlobalPAY Transaction ID: ". $data['ref'] ."\n".
                        "GlobalPAY Payment Status: Paid\n".
            ...
            case 'failed':
                $this->log( '- Failed' );
                $order->update_status( 'failed', sprintf( __( 'Payment %s via ITN.', 'woocommerce-gateway-globalpay' ), strtolower( sanitize_text_field( 'failed' ) ) ) );
               ...
               // if ( $this->send_debug_email ) {
                    ...
                    $body =
                        ...
                        "GlobalPAY Transaction ID: ". $returnedRef ."\n".
                        "GlobalPAY Payment Status: Failed";
                    wp_mail( $pfDebugEmail, $subject, $body );
            ...
            case 'pending':
                ...
                $order->update_status( 'on-hold', sprintf( __( 'Payment %s via GlobalPAY.', 'woocommerce-gateway-globalpay' ), strtolower( sanitize_text_field('Pending' ) ) ) );
                break;
            default:
    ...
    public function confirmTransaction($reff)
    {
        $confirmURL = "https://www.zenithbank.com.gh/api.globalpay/service/confirmTransaction?gpid={{YourID}}&ref=".$reff;
        $response="pending";
        $ch = curl_init();
        ...
        {
            //process response json_decode. this is a receipt of the payment.
